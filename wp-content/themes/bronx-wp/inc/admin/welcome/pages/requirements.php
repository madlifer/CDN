<?php
	...
	// WP Remote Get Check
	$posting['wp_remote_get']['name'] = esc_html__( 'Remote Get', 'bronx' );
...
<table class="widefat" cellspacing="0">
	<thead>
		<thead>
			<tr>
				<td><?php esc_html_e( 'Requirements', 'bronx' ); ?></td>
				<td></td>
			</tr>
		</thead>
		<?php if ( function_exists( 'ini_get' ) ) : ?>
			<tr>
				<td data-export-label="Server Memory Limit">
					<?php esc_html_e( 'Server Memory Limit', 'bronx' ); ?>:
				</td>
				<td>
					<?php
					...
					<?php else: ?>
						<mark class="<?php echo esc_attr($mark); ?>">
								...
								<?php esc_html_e('The recommended value is 256M.', 'bronx' ); ?>
						</mark>
					<?php endif; ?>
			...
			<tr>
				<td data-export-label="PHP Version"><?php esc_html_e( 'PHP Version:', 'bronx' ); ?></td>
				<td>
					<?php
					...
					?>
						...
						<?php else: ?>
							<mark class="<?php echo esc_attr($mark); ?>">
									...
									<?php wp_kses_post('Required PHP Version is at least 5.6. WordPress <a href="https://wordpress.org/about/requirements/" target="_blank">recommends using at least PHP 7.0.</a> ', 'bronx' ); ?>
							</mark>
						<?php endif; ?>
					<?php	} ?>
				</td>
			...
			<tr>
				<td data-export-label="PHP Time Limit"><?php esc_html_e( 'PHP Time Limit', 'bronx' ); ?>:</td>
				<td>
					<?php
					...
					<?php else: ?>
						<mark class="<?php echo esc_attr($mark); ?>">
								...
								<?php esc_html_e('The recommended value is 180.', 'bronx' ); ?>
						</mark>
					<?php endif; ?>
		...
		<?php endif; ?>
		<tr>
			<td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'bronx' ); ?>:</td>
			<td>
				<?php
				...
				<?php else: ?>
					<mark class="<?php echo esc_attr($mark); ?>">
							...
							<?php esc_html_e('The recommended value is 12M.' , 'bronx' ); ?>
					</mark>
				<?php endif; ?>
			</td>
		</tr>
		<?php
			...
			$posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'bronx' ) . '">[?]</a>';
		...
		?>
		<tr>
			<td data-export-label="XML Reader"><?php esc_html_e( 'XML Reader', 'bronx' ); ?>:</td>
			<td>
				<?php
				...
				<?php if ($mark == 'yes'): ?>
					<mark class="<?php echo esc_attr($mark); ?>">
							<span class="dashicons dashicons-yes"></span> <?php esc_html_e('Installed' , 'bronx' ); ?>
					</mark>
				<?php else: ?>
					<mark class="<?php echo esc_attr($mark); ?>">
							<span class="dashicons dashicons-warning"></span>
							<?php esc_html_e('Please install XMLReader extension for your PHP.' , 'bronx' ); ?>
					</mark>
				<?php endif; ?>
			</td>
		</tr>
