<?php
	...
	$header_style = (ot_get_option( 'header_style') !== 'off' ? 'smooth_scroll' : '');
...
<?php if ($header !== 'off') { ?>
	...
	<!-- Start Header -->
	<header class="header style2 <?php echo esc_attr($header_transparent.' '.$header_color.' '.$fixed_header);?> style2" data-offset="400" data-stick-class="header--slide" data-unstick-class="header--unslide">
		<div class="row">
	...
	<?php if (thb_header_container($header_container)) { ?>
			<div id="page-title" class="table">
				<div>
					...
					?>
						<p>
							<?php esc_html_e('Logged in as:', 'bronx' ); echo esc_html($current_user->display_name); ?>
						</p>
					<?php } ?>
				</div>
			</div>
