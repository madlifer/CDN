...
class thb_twitter extends WP_Widget {
	function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__('Display Your Twitter Feed', 'bronx' )
		);

		parent::__construct(
			'thb_twitter_widget',
			esc_html__( 'Bronx - Twitter' , 'bronx' ),
			$widget_ops
		);
    ...
    $tweets = thb_gettweets($params['show'], $params['username']);
		...
		<div class="thb-twitter-carousel-wrapper">
			...
			<div class="thb-carousel slick" data-columns="1" data-fade="true" data-autoplay="1">
		...
		<div class="thb-twitter-content">
			<div class="thb-twitter-user">
				...
				<div class="thb-twitter-usermeta">
					<span><?php echo thb_numberAbbreviation($twitter_data['following']); ?> <?php esc_html_e('Following', 'bronx' ); ?></span>
					<span><?php echo thb_numberAbbreviation($twitter_data['followers']); ?> <?php esc_html_e('Followers', 'bronx' ); ?></span>
				</div>
			</div>
	...
	function form($instance) {
		...
		?>
			...
			<p>
				...
				<select name="<?php echo esc_attr( $this->get_field_name( 'layout' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>" class="widefat">
					<option value="list" <?php selected( $params['layout'], 'list' ); ?>><?php esc_html_e( 'List', 'bronx' ); ?></option>
      ...
      <p>
				<label for="<?php echo esc_attr($this->get_field_id( 'show' )); ?>"><?php esc_html_e('Number of Tweets:', 'bronx' ); ?></label>
