...
function thb_getInstagramPhotos_json( $number = 6, $username, $access_token ) {
	...
	// Check Access Token
	if ( ! $access_token ) {
		$result['error'] = esc_html( 'Please enter an Instagram Access Token.', 'bronx' );
		return $result;
	}
	// Check Username
	if ( ! $username ) {
		$result['error'] = esc_html( 'Please enter an Instagram User.', 'bronx' );
		return $result;
	}
...
function thb_getInstagramPhotos_html( $number = 6, $username ) {
    ...
    $ins_data_json = array_shift( $ins_data );
		...
		if ( ! $ins_data_json ) {
			return esc_html__( 'Instagram html data cannot be retrieved.', 'bronx' );
		}
		$image_data = false;
    ...
    if ( $ins_data_json ) {
      $instagram_json = json_decode( $ins_data_json, true );
			...
			if ( ! isset( $instagram_json['entry_data']['ProfilePage'][0]['graphql']['user'] ) ) {
				return esc_html__( 'Instagram data is not set, please check the ID.', 'bronx' );
			}
			...
			if ( ! isset( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
				return esc_html__( 'There are no images in your account or the account is private.', 'bronx' );
			}
      // Live Count.
      if ( ! empty( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
        $image_data = $thb_user_data['edge_owner_to_timeline_media']['edges'];
      } else {
        $result['error'] = esc_html__( 'There are no images in your account or the account is private.', 'bronx' );
      }
      ...
      $result['error'] = esc_html__( 'Instagram html data cannot be retrieved.', 'bronx' );
    }
		if ($image_data) {
